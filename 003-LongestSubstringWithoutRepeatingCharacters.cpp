...
*/
/*
我们还是建立一个256位大小的整型数组来代替哈希表，这样做的原因是ASCII表共能表示256个字符，所以可以记录所有字符，然后我们需要定义两个变量max_len和start，其中max_len用来记录最长无重复子串的长度，start指向该无重复子串左边的起始位置，然后我们遍历整个字符串，对于每一个遍历到的字符，如果哈希表中该字符串对应的值为-1，说明没有遇到过该字符，则此时将其位置记录last中，如果遇到相同的字符，则其上次为值>=start 则此时计算max_len, start向字符上次出现的位置右移一位，last更新! 
*/
class Solution {
public:
